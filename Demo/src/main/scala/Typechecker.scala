	...
	def typecheck(expr: Expr, env: Map[Identifier, Type]): Option[Type] = 
		expr match {
			...
			case Lambda(params, body) => {
				def helperNewEnv(p: List[(Identifier, Type)], 
					currEnv: Map[Identifier, Type]): Map[Identifier, Type] = 
						p match {
							case Nil() => currEnv
							case Cons(head, tail) => helperNewEnv(tail, currEnv updated (head._1, head._2))						
						}

				val newEnv = helperNewEnv(params, env)
				val paramsTypes = params.map(_._2)
				typecheck(body, newEnv).map(FunctionType(paramsTypes, _))
			}
			case Application(callee, args) => 
				typecheck(callee, env) match {
					case Some(FunctionType(from, to)) => 
						if (args.size == from.size &&
							args.map(typecheck(_, env)).zip(from)
							.forall{case (aType, f) => aType.nonEmpty && aType.get == f}) 
						Some(to) else None()
					case _ => None() 
				}
